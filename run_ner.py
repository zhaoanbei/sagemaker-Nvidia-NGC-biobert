...
# # install('git+https://github.com/NVIDIA/dllogger')
    
# install('nvidia_ml_py3')
...
flags.DEFINE_bool(
    "do_export", False, 
    "Whether to export model to pb format.")
...
#     dllogging = utils.dllogger_class.dllogger_class(FLAGS.dllog_path)
    ...
    if FLAGS.horovod:
      print('SM_NUM_GPUS',int(os.environ["SM_NUM_GPUS"]))
    ...
    if FLAGS.do_train:
        ...
        if FLAGS.horovod:
          ...
          tf.compat.v1.logging.info("  hvd.size() = %d", hvd.size())
...
#         dllogging.logger.log(step=(), data={"throughput_val": ss_sentences_per_second}, verbosity=Verbosity.DEFAULT)
    ...
    print('FLAGS.do_export',FLAGS.do_export)
    if FLAGS.do_export:
        print('FLAGS.do_export',FLAGS.do_export)
        serving_input_fn = serving_input_fn_builder(seq_length=FLAGS.max_seq_length)
        exporter = tf.estimator.FinalExporter('Servo', serving_input_receiver_fn=serving_input_fn)
        exporter.export(estimator, os.path.join(FLAGS.output_dir, 'export/Servo'), os.path.join(FLAGS.output_dir, 'model.ckpt-333'), None, True)
